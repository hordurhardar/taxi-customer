...
class RemoteService {
  Future<http.Response?> callDeleteApi({
    ...
    try {
      ...
      final response = await http.delete(Uri.parse('$BASE_URL$url'), headers: header);
      log('Api response >>> : ${response.body.toString()}');
      log('Api response >>> : ${response.statusCode.toString()}');
      responseJson = _returnResponse(response);
      log('Api header : $header');
    } on SocketException catch (exception) {
      showSnackBar(context: context, isSuccess: false, message: exception.message.toString());
      throw NoInternetException('No Internet');
    } catch (e) {
      log('main catch error $e');
      final exceptionData = jsonDecode(e.toString());
      showSnackBar(context: context, isSuccess: false, message: exceptionData['message'].toString());
  ...
  Future<http.Response?> callGetApi({
    ...
    try {
      ...
      final response = await http.get(Uri.parse(forLocation ? url : '$BASE_URL$url'), headers: header);
      log('Api Url : $BASE_URL$url');
      ...
      if (exception.message.toString().contains('jwt')) {
        showSnackBar(context: context, message: AppLocalizations.of(context!)!.sessionExpired, isSuccess: false);
        logOut(context: context);
      } else {
  ...
  Future<http.Response?> callMultipartApi({
    ...
    var authToken = await getAuthToken();
    var osType = await getOsType();
    var request = http.MultipartRequest(requestName ?? 'POST', Uri.parse(BASE_URL + url));
    ...
    if (fileParamName != null && file != null) {
      request.files.add(http.MultipartFile(fileParamName, file.readAsBytes().asStream(), file.lengthSync(), filename: file.path.split("/").last));
    }
 ...
 //       logOut(context: context);
  ...
  Future<http.Response?> callPostApi({
    ...
    try {
      ...
      log(authToken ?? "");
      var osType = await getOsType();
      final response = await http.post(Uri.parse(isForStripe ? '$urlForStripe' : '$BASE_URL$url'),
 ...
 //       logOut(context: context);
  ...
  Future<http.Response?> callPutApi({
    ...
    try {
      ...
      final response = await http.put(Uri.parse('$BASE_URL$url'), headers: header, body: jsonEncode(jsonData));
      responseJson = _returnResponse(response);
      log('Api header : $header');
    } on SocketException catch (exception) {
      if (exception.message.toString().contains('jwt')) {
